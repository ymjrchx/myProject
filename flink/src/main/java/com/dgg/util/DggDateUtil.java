...
public class DggDateUtil {
     ...
     * 默认秒格式
     */
    public static final String DEFAULT_TIMESTAMP_SECOND = "yyyyMMddHHmmss";
    /**
     * 默认毫秒格式
     */
    private static final String DEFAULT_TIMESTAMP_MILLISECOND = "yyyyMMddHHmmssSSS";

    private static SimpleDateFormat sdf = new SimpleDateFormat(DEFAULT_DATE_FORMATTER);
     ...
     *根据时间类型增加时间
     * @param date 时间
     * @param type 时间单位  天、时、分、秒等
     * @param number 增加的天数
     * @return
     */
    public static Date dataAddNumber(Date date, int type, int number) {
    ...
    /**
     * 获取今天凌晨的日期,格式yyyyMMdd
     * @return
     */
    public static String getDayDate(){
        long now = System.currentTimeMillis() ;
        long daySecond = 60 * 60 * 24 * 1000;
        long dayTime = now - (now + 8 * 3600*1000) % daySecond;
        String format = sdf1.format(dayTime);
        return format;
    }
    /**
     * 获取明天凌晨的日期时间
    ...
    public static String getNextDate(){
        long now = System.currentTimeMillis() ;
        long daySecond = 60 * 60 * 24 * 1000;
        long dayTime = now - (now + 8 * 3600*1000) % daySecond + daySecond;
        String format = sdf.format(dayTime);
        return format;
    }
    ...
    public static String getThreeDate(){
        long now = System.currentTimeMillis() ;
        long daySecond = 60 * 60 * 24 * 1000;
        long dayTime = now - (now + 8 * 3600*1000) % daySecond + daySecond * 2;
        String format = sdf.format(dayTime);
        return format;
    }
    /**
    ...
    public static String getFourDate(){
        long now = System.currentTimeMillis() ;
        long daySecond = 60 * 60 * 24 * 1000;
        long dayTime = now - (now + 8 * 3600*1000) % daySecond + daySecond * 3;
        String format = sdf.format(dayTime);
        return format;
    }
    ...
    public static String getMonth(){
        long now = System.currentTimeMillis();
        String format = sdfYM.format(now);
        return format;
    }

    public static void main(String[] args) {
        System.out.println(getDayDate());
        System.out.println(getNextDate());
        System.out.println(getThreeDate());
        System.out.println(getFourDate());
    }
}
